...
> 2026-04-10 · 프론트만 하다가 처음으로 DB를 해봤다

솔직히 처음엔 왜 DB를 직접 배워야 하나 싶었다.
fetch 해서 데이터 받으면 되는 거 아닌가? 근데 막상 해보니까 데이터가 어떻게 저장되는지 모르면 설계 자체를 이해 못 하겠더라.
낯설었지만 풀면서 조금씩 감이 왔다. 나중에 다시 볼 것 같아서 기록해둠.
...
테이블을 나눠야 한다는 게 처음엔 왜인지 잘 몰랐다. 근데 같은 크루가 출석할 때마다 이름이랑 id가 계속 반복 저장된다는 걸 보고 이해했다. "중복이 발생한다"는 게 이런 거구나 싶었음.
...
crew 테이블을 따로 만들었으니까 이제 attendance에서 nickname을 들고 있을 필요가 없어졌다. 근데 `DROP COLUMN`이 진짜 삭제라는 게 좀 무서웠다. 실제로 날아가는 거니까.
...
"DB 레벨에서 차단된다"는 게 흥미로웠다. 프론트에서만 유효성 검사하는 게 아니라 DB 자체가 막아주는 구조가 있다는 게 신기했음.
...
같은 닉네임이 두 개면 누가 누군지 알 수가 없으니까 unique 제약이 필요하다는 건 바로 이해했다.
...
CRUD는 프론트에서도 많이 들어본 개념이라 이건 좀 친숙하게 느껴졌다.

### 문제 5: 크루 닉네임 검색하기 (LIKE)

`%`가 와일드카드라는 건 바로 이해됐다. `'디%'`는 "디"로 시작하는 것들 전부.
...
서브쿼리를 처음 써봤다. WHERE 안에 SELECT가 또 들어가는 구조인데, 읽을 때 좀 어색했다. 익숙해지면 괜찮을 것 같긴 한데.
...
외래키 제약이 있으니까 crew에 없는 crew_id로는 attendance에 넣을 수가 없다. 순서를 지켜야 한다는 게 포인트였음.
...
WHERE 없이 UPDATE 하면 전체 행이 다 바뀐다는 거 듣고 좀 무서워졌다. 항상 WHERE 쓰는 습관 들여야겠다고 생각했음.
...
DELETE도 마찬가지. WHERE 없으면 전부 날아간다.
...
JOIN이 제일 어려웠다. 두 테이블을 어떤 기준으로 연결하는지 감을 잡는 데 시간이 걸렸음. `ON a.crew_id = c.crew_id` 부분이 "이 컬럼을 기준으로 붙여라"는 의미라는 걸 이해하고 나서야 읽혔다.
...
서브쿼리 한 번 더 연습. 이번엔 들여쓰기로 좀 더 읽기 편하게 써봤다.
...
`ORDER BY ... DESC LIMIT 1` 조합. 자바스크립트에서 sort 하고 `[0]` 꺼내는 것이랑 비슷한 느낌이었다.
...
GROUP BY가 처음엔 낯설었는데, "그룹별로 뭔가를 계산한다"고 생각하니까 이해됐다.

### 문제 13: 크루별로 '기록된' 날짜 수 조회

crew_id 기준으로 묶고, 각 그룹의 행 수를 센다.
...
`COUNT(*)`랑 `COUNT(컬럼)` 차이가 NULL 처리에서 나온다는 걸 여기서 처음 알았다. `COUNT(컬럼)`은 NULL을 빼고 센다.
...
이번엔 GROUP BY 기준이 crew_id가 아니라 날짜.
...
MIN이랑 MAX를 한 쿼리에서 같이 쓸 수 있다는 게 편했다.
...
SQL 문법보다 "왜 이렇게 설계했는지"를 설명하는 게 더 어려웠다. 코드는 틀리면 에러가 나는데, 설계 이유는 내가 제대로 이해한 건지 확인이 안 되니까.
...
심화 파트는 읽으면서 "아 그렇구나" 하는 것들이 많았는데, 특히 성능 관련 내용은 아직 직접 체감하기가 어려웠다. 나중에 실제로 데이터 많아지면 다시 봐야 할 것 같다.
